  ...
  ## Trends over time in key demographic indicators
    ...
    # Plot for each indicator  
      ...
      for (i in 1:length(indicators) )  {
       ...
       x2[, "admin1"] <- as.character(x2[, "admin1"])
       plot <- ggplot(subset(x2, y >= y_excess_start), aes(x = date_recall, y = var, group = survey_id) )
       plot <- plot + geom_point( aes(color = admin1), alpha = 0.6 ) + 
         geom_line( aes(color = admin1), alpha = 0.6 ) + 
         ...
         theme(plot.title = element_text(color="grey30"),
          ...
          legend.title = element_text(color="grey30", size = 9),           
          )
      ...
      # Assign plot name and save
        ...
        assign(x9, plot)
        # save
        ggsave(x9, height = 10, width = 20, units = "cm", dpi = "print")
